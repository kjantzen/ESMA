...
handles.menu_copyfiles = uimenu(handles.menu_file, 'Label', 'Duplicate', 'Accelerator', 'c');
...
handles.menu_arbadchans = uimenu('Parent', handles.menu_preprocess, 'Label', 'Automatically clean data');
...
%context menu
handles.cm = uicontextmenu(handles.figure);
handles.scm(1) = uimenu(handles.cm, "Text", "Plot");
handles.scm(2) = uimenu(handles.cm, "Text", "Rename");
handles.scm(3) = uimenu(handles.cm, "Text", "Delete");
handles.scm(4) = uimenu(handles.cm, "Text", "Duplicate");
...
set(handles.menu_copyfiles, 'Callback', {@callback_copyfiles, handles});
...
if ~isempty(filestorename)
    ...
    try
        ...
        if strcmp(resp.option, 'Accept') && ~isempty(resp.input)
            ...
            eeg_UpdateProcessLog(study, "ColumnNames",columnName, ...
...
%**************************************************************************
%deletes selected files from the disk
function callback_deletefiles(~, ~, h)
...
start = clock;
reportValues = eeg_Biosemi2EEGLab(fnames,'Chanlocs', study.chanlocs, ...
    'AvgRef', 0, 'ApplyFilt', 0, 'Lpass', 0, 'Hpass', 0, 'OWrite', 2, ...
    'FileExt', '.cnt', 'FigHandle', h.figure);


parameters.duration = {'Duration', toc};
eeg_UpdateProcessLog(study, 'ColumnNames',columnNames, 'RowNames', fnames,...
    'SheetName','biosemi convert', 'Parameters',parameters, 'Values',reportValues);

study = study_AddHistory(study, 'start', start, 'finish', clock,'event', 'Conversion to EEGLAB format', 'function', 'eeg_Biosemi2EEGLab', 'paramstring', fnames, 'fileID', '.cnt');
...
pb.Message = sprintf('Applying average reference to all participants');
...
for ii = 1:length(fnames)
    ...
    eeg_SaveEEGFile(EEG, fullfile(path, [newfile, ext]));
    pb.Value = ii/length(fnames);
end

parameters.duration = {'Duration', toc};
eeg_UpdateProcessLog(study,"ColumnNames",reportColumnNames,...
...
for ii = 1:length(fnames)
    ...
    eeg_SaveEEGFile(EEG, fullfile(path, newfile));
    pb.Value = ii/length(fnames);
end
...
%*************************************************************************
function callback_interpchans(~, ~, h)
tic
...
eeg_UpdateProcessLog(study,'SheetName', 'rem chans', ...
...
if ~eeg_ValidateFileTypes(selfiles, {'.cnt'})
...
eeg_UpdateProcessLog(study,'SheetName', 'rem time', ...
