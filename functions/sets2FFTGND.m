...
GND.filepath=[];
GND.saved='no';
GND.power_grands=zeros(n_chans,power_pts,n_bins)*NaN;
GND.power_grands_stder=GND.power_grands;
GND.phase_grands = GND.power_grands;
GND.phase_grands_stder = GND.power_grands;
GND.itc_grands = GND.power_grands;
GND.itc_grands_stder = GND.power_grands;

GND.grands_t=GND.power_grands;
...
GND.freq_pts = EEG.srate/fft_pts * (0:power_pts-1);
...
GND.indiv_power=zeros(n_chans,power_pts,n_bins,n_infiles);
GND.indiv_phase = GND.indiv_power;
GND.indiv_itc = GND.indiv_power;
        ...
        for b=1:length(EEG.epoch(a).eventtype),
            if (length(EEG.epoch(a).eventtype{b})>2) && strcmpi(EEG.epoch(a).eventtype{b}(1:3),'bin')
                ...
                if ismember(bin,use_bins) && ~index_cellarray_or_vector(EEG.epoch(a).eventlatency,b,VERBLEVEL),
                    ...
                    if new_event
                        %get the complex dft
                        Y = fft(EEG.data(use_chans,:,a), fft_pts, 2)./fft_pts;
                        %calculate the power for the positive side of the
                        %spectrum
                        P = Y.*conj(Y);
                        P = P(:,1:power_pts);
                        P(:,2:end) = 2*P(:,2:end);
                        %normalize the amplitude to 1 and add to the sum
                        ITC = Y./abs(Y);
                        %assign the values
                        GND.indiv_power(:,:,binind,sub)=GND.indiv_power(:,:,binind,sub)+P; % TPU bin->binind
                        GND.indiv_phase(:,:,binind,sub)=GND.indiv_phase(:,:,binind,sub)+ angle(Y(:,1:power_pts));
                        GND.indiv_itc(:,:,binind,sub)=GND.indiv_itc(:,:,binind,sub)+ ITC(:,1:power_pts);
...
GND.indiv_power=GND.indiv_power(:,:,:,1:sub_ct);
GND.indiv_phase=GND.indiv_phase(:,:,:,1:sub_ct);
GND.indiv_itc=GND.indiv_itc(:,:,:,1:sub_ct);
...
for sub=1:sub_ct,
    ...
    for b=1:n_bins,
        ...
        if bin_ct,
            ...
            GND.indiv_power(:,:,b,sub)=GND.indiv_power(:,:,b,sub)/bin_ct;
            GND.indiv_phase(:,:,b,sub)=GND.indiv_phase(:,:,b,sub)/bin_ct;
            GND.indiv_itc(:,:,b,sub)=abs(GND.indiv_itc(:,:,b,sub))/bin_ct;
            
            GND.sub_ct(b)=GND.sub_ct(b)+1;
        else
            watchit(sprintf('Subject %s has no epochs that fall into bin %d.',GND.indiv_subnames{sub},b));
            GND.indiv_power(:,:,b,sub)=GND.indiv_power(:,:,b,sub)*NaN;
            GND.indiv_phase(:,:,b,sub)=GND.indiv_phase(:,:,b,sub)*NaN;
            GND.indiv_itc(:,:,b,sub)=GND.indiv_itcGND(:,:,b,sub)*NaN;
        end
    end
end
...
GND.power_grands = mean(GND.indiv_power,4);
GND.phase_grands = mean(GND.indiv_phase, 4);
GND.itc_grands = mean(GND.indiv_itc, 4);
GND.power_grands_stder = std(GND.indiv_power, 0, 4)./sqrt(sub_ct);
GND.phase_grands_stder = std(GND.indiv_power, 0, 4)./sqrt(sub_ct);
GND.itc_grands_stder = std(GND.indiv_itc, 0, 4)./sqrt(sub_ct);
...
elseif ~strcmpi(p.Results.out_fname,'no save'),
    %[jpath, jname, ~]=fileparts(p.Results.out_fname);
    %outfile = fullfile(jpath, jname);
    fprintf('Saving GND variable as %s\n',p.Results.out_fname);
    save(p.Results.out_fname,"GND", "-mat");
%    GND=save_matmk(GND,jname,jpath,force);
%    movefile(fullfile(jpath, jname), fullfile(jpath, newname));
end
