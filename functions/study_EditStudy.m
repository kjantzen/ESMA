...
%*************************************************************************
function map = erpColorMap(mapname)

    if nargin == 0
        mapname = 'gem12';
    end
    map = orderedcolors(mapname);
    %map the map longer if it is less than 8
    if length(map) < 8
        map = repmat(map, 2, 1);
    end
...
for cntrl = h.button_tabselect
    cntrl.ButtonPushedFcn = {@callback_toggletabs, h};
end

h.dropdown_erp_palette.ValueChangedFcn = {@callback_addPredefindERPPalette, h};
for ii = 1: 8
...
h.bg = uibuttongroup('Parent', h.figure,...
    'Position', [10, 375,502,27],...
    'BackgroundColor',scheme.Panel.BackgroundColor.Value,...
    'BorderType','none');

tab_text = {'General', 'Subject', 'Epoch Bins', 'Channel Group','Plot Colors'};
for ii = 1:length(tab_text)
    xpos = (ii-1) * 100 +1 ;
    h.button_tabselect(ii) = uibutton('Parent', h.bg,...
        'Text', tab_text{ii},...
        'Position', [xpos, 1, 100, 25],...
...
h.button_tabselected(1).Tag = 'Selected';
...
for ii = 1:8
    ...
    uilabel( 'Parent', h.color_select_panel,...
        'Position', [150, btm, 120, 20],...
        'BackgroundColor', scheme.Window.BackgroundColor.Value,...
        'FontName',scheme.Label.Font.Value,...
        'FontColor',scheme.Label.FontColor.Value,...
        'FontSize',scheme.Label.FontSize.Value,...
        'Text', sprintf('ERP condition %i', ii));
end
...
colorpalette_names = {'gem12','glow12','sail','reef','meadow','dye','earth'};
