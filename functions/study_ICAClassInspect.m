...
if bad_status
    ...
    mapTitle = [mapTitle, ' GOOD'];
    bcolor = p.scheme.Window.BackgroundColor.Value;
end

cla(h.axis_icatopo);
wwu_topoplot(ic,  p.EEG.chanlocs, 'axishandle', h.axis_icatopo, 'backcolor', bcolor);
cb = colorbar(h.axis_icatopo);
cb.Color = p.scheme.Axis.AxisColor.Value;

h.axis_icatopo.Title.String = mapTitle;
h.axis_icatopo.Title.Color = p.scheme.Label.FontColor.Value;
h.axis_icatopo.Title.FontSize = 14;
...
if p.EEG.trials > 1
    ...
    h.axis_icaerp.Title.FontSize = 14;
...
for ii = 1:length(pbar_data)
    b = bar(h.axis_compbar, ii, pbar_data(ii), 'BarWidth', .9);
...
callback_plotclassifications([],[],h, comp);
    ...
    %outline the selected component
     for ii = 2:size(thresh_w,1)
      ...
      selComp(2) = rectangle(h.axis_compclassthresh, 'Position', [find(ic_indx==curr_ic) - .5, 0, 1, h.axis_compclassraw.YLim(2)], 'EdgeColor', 'w', ...
            'FaceColor','none', 'LineWidth',2);
      h.axis_compclassraw.UserData = selComp;
else
    %nothing else has changed so only change the position of the selection
    %rectangles
    selComp = h.axis_compclassraw.UserData;
    selComp(1).Position(1) = curr_ic - .5;
    selComp(2).Position(1) = find(ic_indx==curr_ic)-.5;
    h.axis_compclassraw.UserData = selComp;
end
...
%**************************************************************************
function handles = buildGUI(scheme)
