...
callback_relfreshDisplay([],[],handles, false, false)
...
function callback_relfreshDisplay(~, ~, h, reload_flag, newfile_flag)
...
if reload_flag  
    if newfile_flag
        [new_file, new_path] = uigetfile({'*.GND'}, 'Load ERP file', p.GND.filepath);
        if new_file == 0
            return
        end
        erp_filename = fullfile(new_path, new_file);
    else
...
handles.menu_opennew.MenuSelectedFcn = {@callback_relfreshDisplay, handles, true, true};
handles.menu_refresh.MenuSelectedFcn = {@callback_relfreshDisplay, handles, true, false};
