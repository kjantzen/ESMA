...
%
function study_PlotFFT(study, filename)
%study_PlotFFT(study, filename)
%
% Plotting and analysis interface for FFT GND files
% Inputs:
%   study - an esma study structure
%   filename - the name of a GND FFT file to laod and display
%
...
handles.axis_fft.UserData = cinfo;
...
%% TOGGLE FUNCTIONS FOR HANDLING WHEN USER CHANGES OPTIONS
%***************************************************************************
...
function callback_toggleautoscale(hObject, event, h)
    ...
    callback_plotchannels(hObject, event, h)
%************************************************************************
...
function callback_toggleallchannel(hObject, event, h)
    ...
    callback_plotchannels([],[],h);
%*************************************************************************
%function to handle when user changes the slot type
function callback_togglePlotType(hObject, ~, h)
    for ii = 1:4
        h.menu_FFTTypeOption(ii).Checked  = false
    end
    hObject.Checked = true;
...
function callback_changePlotRange(hObject, event, h)
    ...
    mnt = h.spinner_minfreq.Value;
    mxt = h.spinner_maxfreq.Value;
    mna = h.spinner_minamp.Value;
    mxa = h.spinner_maxamp.Value;
    if mnt >= mxt
        p = h.figure.UserData;
        if mxt == p.GND.freq_pts(end) || strcmp(tag, 'maxtime')
            mnt = mxt - h.spinner_minfreq.Step;
            h.spinner_minfreq.Value = mnt;
        elseif mnt == p.GND.freq_pts(1) || strcmp(tag, 'mintime')
            mxt = mnt + h.spinner_maxfreq.Step;
            h.spinner_maxtfreq.Value = mxt;
        end
    end
    if mna >=mxa
        mna = mxa - 1;
    ...
    callback_plotchannels(hObject,event,h);
%% GET FUNCTIONS TO READ STATUS OF CONTROLS
%*************************************************************************
function type = getPlotType(h)
    for ii = 1:4
        if h.menu_FFTTypeOption(ii).Checked
            type = h.menu_FFTTypeOption(ii).Tag;
            break
        end
    end
%*************************************************************************
function quality = getMapQuality(h)
    %lowest res allowed
    baseQuality = 40;
    for ii = 1:4
        if h.menu_mapquality(ii).Checked
            quality = baseQuality * ii;
            break
        end
    end
%*************************************************************************
function scaleType = getMapScaleType(h)
...
function m = cond2StyleTable(stat) 
    ...
    m.condition = stat.conditions';function callback_plotANOVAresult(hObject, event,h, export)
...
if reload_flag
 ...
 %           for ii = 1:length(EEG.F_tests)
 %               if iscell(EEG.F_tests(ii).used_tpt_ids)
 %                   EEG.F_tests(ii).used_tpt_ids = cell2mat(EEG.F_tests(ii).used_tpt_ids);
 %               end
 %           end
 %       end
 %   end
...
switch event.EventName
    ...
    case 'WindowMousePress'
        c.dragging = true;
        h.axis_fft.UserData = c; %save the cursor before making more calls to functinos that load cursor data.
        ...
        h.axis_fft.UserData = c; %save it again to update the cursor
...
%make sure the new cursor point exists in the data
cursor.time = samp_interval * round(new_time/samp_interval);
%get the X posiotion of the cursor shape
curr_Xloc = cursor.polygon.Shape.Vertices(1,1);
%difference between current and new location
delta_loc = cursor.time - curr_Xloc;
%shift cursor by the distance to move it to the new location
cursor.polygon.Shape.Vertices(:,1) = cursor.polygon.Shape.Vertices(:,1) + delta_loc - samp_interval/2;
fprintf('%.2f\n', cursor.time)
...
switch(event.Source.Tag)
    case 'add'
        ...
        c.cursor(enum).time = h.axis_fft.XLim(2)/2;
        c.cursor(enum).width = 10;
        c.cursor(enum).open = false;
        c.cursor(enum).polygon = build_cursor(c.cursor(enum), h);
        c.mincursorstep = p.GND.freq_pts(2);
        c.maxcursorpos = p.GND.freq_pts(end);
        c.mincursorpos = p.GND.freq_pts(1);
...
%draw the cursor on the screen in frequency units
%the size of the cursor is always one bin width and so changes based on how
%zoomed the plot is.
%
axisWidth = h.axis_fft.Position(3);
xl = h.axis_fft.XLim;
yl = h.axis_fft.YLim;
yr = .025 * range(yl);
pwidth = axisWidth / (xl(2)-xl(1));

clr = h.axis_fft.XColor;
%draw the cursor
if pwidth < 4
    rect_x = [cursor.time-.5, cursor.time+.5,cursor.time+.5,...
        cursor.time+1,cursor.time+1,...
        cursor.time-1,cursor.time-1,...
...
hold(h.axis_fft, 'on');
pg = plot(h.axis_fft, ps,  'FaceColor', clr, 'FaceAlpha', .5, 'EdgeColor', 'none', 'EdgeAlpha', 1);
...
if ~isempty(cinfo.currentcursor)
    cinfo.cursor(cinfo.currentcursor).polygon.FaceColor = cinfo.cursor(cinfo.currentcursor).polygon.UserData;
end
if ~isempty(new_cnum) && ~isempty(cinfo.cursor)
    cinfo.cursor(new_cnum).polygon.FaceColor = 'g';
...
%**************************************************************************
function params = selectDataType(GND, params)
        %select the appropraite data
        
        if matches(params.plotType, 'POWER') || matches(params.plotType, 'AMP')
            params.YAxisLabel = 'Amplitude {\mu}V^{2}';
            fieldname = 'power';
        elseif matches(params.plotType, 'PHASE')
            params.YAxisLabel = 'Phase Angle (radians)';
            fieldname = 'phase';
        elseif matches(params.plotType, 'ITC')
            params.YAxisLabel = 'Inter Trial Coherence';
            fieldname = 'itc';
        end
        if params.sbj == 0
            fieldname = [fieldname, "_grands"];
            indx  = 1;
        else
            fieldname = ["indiv_", fieldname];
            indx = params.sbj;
        end

        params.data = GND.(join(fieldname, ""))(:,:,:,indx);
        if matches(params.plotType, 'AMP')
            params.data = sqrt(params.data);
            params.YAxisLabel = 'Amplitude {\mu}V';
        end   
        

%**************************************************************************
function params = getdatatoplot(study, GND, h, cursors, aveBetween)
%initialize variables
params.data = []; params.se = [];
params.stat = [];
params.pstat = [];
params.labels = []; %either channel labels or map labels
params.conditions = [];
params.fixedScale = [];
params.se = [];
p = [];
...
%get some parameters to know what data to send back
params.plotType = getPlotType(h);
params.conditions = h.list_condition.Value;
ch = cell2mat(h.list_channels.Value');
params.channels = ch(find(ch(:,1)),1);
params.sbj = h.list_subject.Value;
mass_univ_overlay = h.check_MUoverlay.Value;

if mapping_mode
    [~,pt] = arrayfun(@(a) min(abs(a-GND.freq_pts)), sort([cursors.time]));
    params.channels = 1:length(GND.chanlocs); %overwrite channel selection if we are mapping
    plot_GFP = false;
else
    pt =1:length(GND.freq_pts); %get all the points 
...
params = selectDataType(GND, params);
%it is possible that no channels are selected because just the channel
%groups can be selected
if aveBetween
    params.data = mean(params.data(params.channels,pt(1):pt(2),params.conditions),2);
else
    params.data = params.data(params.channels,pt,params.conditions);
%    se = GND.grands_within_CI(params.channels, pt, cond_sel);
%    se = GND.power_grands_stder(params.channels, pt, cond_sel);
end
params.fixedScale = max(params.data, [], 'all');
if ~isempty(params.channels)
    if params.sbj==0  && mass_univ_overlay            
            %initialize the array to the full size of the data
             stat = zeros(size(fdata,1), size(fdata,2));
             pstat = stat;
            ...
            if aveBetween
                params.stat = mean(stat(params.channels,pt(1):pt(2)),2);
                params.pstat = mean(pstat(params.channels,pt(1):pt(2)),2);
            else
                params.stat = stat(params.channels, pt); %select the part the user requested
                params.pstat = pstat(params.channels,pt);
            end            
    end    
    if mapping_mode
        params.labels = sort([cursors.time]);
    else
        params.labels = {GND.chanlocs(params.channels).labels};
    end
end
...
if ~isempty(ch_groups) && ~mapping_mode
    ...
    ch_group_data = zeros(length(ch_groups), length(GND.freq_pts), length(params.conditions));
    params.se = ch_group_data;
    ch_group_s = zeros(length(ch_groups), length(GND.freq_pts));
    for ii = 1:length(ch_groups)
        for jj = 1:length(params.conditions)
        ch_group_data(ii,pt,jj) = squeeze(mean(params.data(study.chgroups(ch_groups(ii)).chans,:,params.conditions(jj)),1));
...
%       se(ii,pt,jj) = squeeze(mean(GND.power_grands_stder(study.chgroups(ch_groups(ii)).chans,:,cond_sel(jj)),1));
        if mass_univ_overlay && ~mapping_mode %mapping data for these channels is not valid
            ...
            stat = zeros(size(params.data,1), size(params.data,2));
            pval = adj_pval<r.desired_alphaORq;
            if contains(r.mean_wind, 'yes')
    ...
    %exist
    if ~isempty(params.channels)
        params.data = cat(1, ch_group_data, params.data); 
        params.labels = horzcat({study.chgroups(ch_groups).name}, params.labels);
    else
        params.data = ch_group_data;
        params.labels = {study.chgroups(ch_groups).name};
    end
    %now add the statistical information if needed
    if mass_univ_overlay
        if ~isempty(params.channels)
            params.stat = cat(1, ch_group_s, s);
        else
            params.stat = ch_group_s;
        end
    end
end
...
if plot_GFP
    ...
    if sbj == 0
        gfp = std(GND.power_grands(:,:,params.conditions),0,1);
    else
        gfp = GND.indiv_power(params.channels, pt, params.conditions, sbj);
    end

    params.data = cat(1, params.data, gfp);
    params.labels(end + 1) = {'GFP'}; 
 end
...
gridscale = getMapQuality(h);
averageBetweenCursors = h.menu_cursormean.Checked;
scale_option = getMapScaleType(h);
...
params = getdatatoplot(p.study, p.GND, h, c.cursor, averageBetweenCursors);
if scale_option ==1
    map_scale = max(abs(params.data),[],'all'); 
elseif scale_option == 3 %fixed scale
    map_scale = params.fixedScale;
end

if ~isempty(params.stat)
    params.data(:,:,end+1) = params.stat;
    has_stat = true;
end
...
n_conds = size(params.data,3); %the number of conditions
...
if n_conds==1 || (n_conds>1 && n_maps==1)    
    ...
    if n_conds==1; comp_conds = false; end    
else     
    max_cols = n_conds;
    ncols = max_cols;
    nrows = n_maps;
...
for ii = 1:n_maps
    for jj = 1:n_conds %this will be for comparing conditions        
        ...
        if jj==n_conds && has_stat %this is the statistical map
            ms = [0,max(max(params.data(:,:,n_conds))) * .8];
            ...
            extraChans = find(params.pstat(:,ii));
        else
            if strcmp('PHASE', params.plotType)
                ms = [-map_scale,map_scale];
            else
                ms = [0; map_scale];
            end
            title_string =  h.list_condition.Items{params.conditions(jj)};
            cmap = feval(p.study.Render.Map.Palette);
            extraChans = params.channels;
        end
        ...
        elseif jj==n_conds && has_stat 
            ...
            cb.Color = h.axis_fft.XColor;
        end
        
        if ii==1  && comp_conds
            my_h(pcount).Title.String = title_string;
            my_h(pcount).Title.Interpreter = 'none';
            my_h(pcount).Title.Color = h.axis_fft.XColor;
        end
        if averageBetweenCursors
            my_h(pcount).XLabel.String = sprintf('%.1f-%5.1f Hz', params.labels(1),params.labels(2));
        else
            my_h(pcount).XLabel.String = sprintf('%.2f Hz', params.labels(ii) );
        end
        my_h(pcount).XLabel.Visible = true;
        my_h(pcount).XLabel.Color = h.axis_fft.XColor;       
    end
end
...
if scale_option ~=2
    ...
    cb.Label.String = params.YAxisLabel;
    cb.Color = h.axis_fft.XColor;
end
...
function callback_plotchannels(~, ~, h)
...
mnFreq = h.spinner_minfreq.Value;
mxFreq = h.spinner_maxfreq.Value;
mnAmp = h.spinner_minamp.Value;
mxAmp = h.spinner_maxamp.Value;
ANOVAStyle = h.check_plotANOVAStyle.Value;
plotdb = h.menu_plotdb.Checked;

p = h.figure.UserData;
params = getdatatoplot(p.study, p.GND, h);
%can't plot it if it is not there!
if isempty(params.data)
    return
end
%convert to dB - but only if it makes sense for the data type
if plotdb && ~(matches(params.plotType, 'PHASE')  || matches(params.plotType, 'ITC'))
    params.data = 10 * log10(params.data);
    params.YAxisLabel = [params.YAxisLabel, ' dB' ];
end
[nch, npt, ncond] = size(params.data);
...
legend_names = cell(1,ncond);
legend_handles = [];

% if the user has selected the butter fly plot option where 
% are stacked on the same origin.
%fix this for fft because it does not work well
if ~stacked  && size(params.data, 1) > 1
    if userScale 
    ...
    v = size(params.data,1):-1:1;
    spread_matrix = repmat(v' * spread_amnt, 1, size(d,2), size(params.data,3));
    params.data = params.data + spread_matrix;
end

%main plotting loop - plot the time series for each condition
cla(h.axis_fft);
hold(h.axis_fft, 'on');
for ii = 1:size(params.data,3)
    ...
    dd = squeeze(params.data(:,:,ii));

    %plot the statistics
    if MUoverlay && ~isempty(params.stat)
        hold(h.axis_fft, 'on');
        tt = repmat(p.GND.freq_pts, size(params.stat,1),1);        
        splot = scatter(h.axis_fft, tt(params.stat>0)', dd(s>0)',60,'filled');
        splot.CData =  pColor;
    end

    %plot the standard error overlay
    if ~isempty(params.se) && SEoverlay
        for jj = 1:size(d,1)
        e = squeeze(params.se(jj,:,ii));
            xe = [p.GND.freq_pts, fliplr(p.GND.freq_pts)];
            ye = [dd(jj,:) + e, fliplr(dd(jj,:)-e)];
            er = patch(h.axis_fft,xe, ye,pColor);
            er.FaceColor = pColor;
            er.EdgeColor = 'None';
            er.FaceAlpha = .25;
        end
    ...
    ph = semilogy(h.axis_fft, p.GND.freq_pts, dd', 'Color',pColor,...
        'LineWidth', lwidth,...
        'LineStyle',pStyle);
    hold(h.axis_fft, 'on');
    for phi = 2:length(ph)
    ...
    if matches(params.labels{end}, 'GFP')
        ph(end).LineWidth = ph(end).LineWidth + .5;
        ph(end).Color = [0,.8,0];
    end
    legend_handles(ii) = ph(1);
    legend_names(ii) = h.list_condition.Items(params.conditions(ii));
end
hold(h.axis_fft, 'off');    
%handle axes and scaling differently depending on whether the plot is
%stacked or not
if ~stacked && size(params.data, 1) > 1
    h.axis_fft.YLim = [min(d, [], 'all') - (spread_amnt * .1), max(d, [], 'all') + (spread_amnt * .1)];
    h.axis_fft.YTick = sort(spread_matrix(:,1));
    h.axis_fft.YTickLabel = labels(v);
    h.axis_fft.YLabel.String = [params.YAxisLabel,' x channel'];
else
    if userScale 
        h.axis_fft.YLim = [mnAmp, mxAmp]; 
    else
        h.axis_fft.YLim = [min(params.data, [], 'all') * 1.1, max(params.data, [], 'all') * 1.1];
    end
    l = line(h.axis_fft, h.axis_fft.XLim, [0,0],...
        'Color', [.5,.5,.5], 'LineWidth', 1.5);
    l.Annotation.LegendInformation.IconDisplayStyle = 'off';    
    h.axis_fft.YTickMode = 'auto';
    h.axis_fft.YTickLabelMode = 'auto';
    h.axis_fft.YLabel.String = params.YAxisLabel;        
end
h.axis_fft.XGrid = 'on'; h.axis_fft.YGrid = 'on';
h.axis_fft.XLim = [mnFreq, mxFreq];
h.axis_fft.XLabel.String = 'Frequency (Hz)';
h.axis_fft.YDir = 'normal';
h.axis_fft.FontSize = 14;


%draw a vertical line at 0 ms;
%time_lock_ms = min(abs(p.GND.freq_pts));
%l = line(h.axis_fft, [time_lock_ms, time_lock_ms], h.axis_fft.YLim,...
%    'Color', [.5,.5,.5], 'LineWidth', 1.5);
%l.Annotation.LegendInformation.IconDisplayStyle = 'off';
...
lg = legend(h.axis_fft, legend_handles, legend_names, 'box', 'off', 'Location', 'NorthOutside', 'NumColumns', legend_columns,'Interpreter', 'none');
lg.Color ='none';
lg.TextColor = h.axis_fft.XColor;
lg.LineWidth = 2;
lg.FontSize = 14;
...
handles.spinner_minfreq.Value = GND.freq_pts(1);
handles.spinner_minfreq.Limits = [GND.freq_pts(1),  GND.freq_pts(end)];
handles.spinner_minfreq.Step = diff(GND.freq_pts(1:2));
   
handles.spinner_maxfreq.Value = GND.freq_pts(end);
handles.spinner_maxfreq.Limits = [GND.freq_pts(1),  GND.freq_pts(end)];
handles.spinner_maxfreq.Step = diff(GND.freq_pts(1:2));

handles.edit_massunivstart.Limits = [GND.freq_pts(1),  GND.freq_pts(end)];
handles.edit_massunivstart.Value = GND.freq_pts(1);

handles.edit_massunivend.Limits = [GND.freq_pts(1),  GND.freq_pts(end)];
handles.edit_massunivend.Value = GND.freq_pts(end);
...
handles.spinner_lwidth.ValueChangedFcn = {@callback_plotchannels, handles};
handles.spinner_minfreq.ValueChangedFcn = {@callback_changePlotRange, handles};
handles.spinner_maxfreq.ValueChangedFcn = {@callback_changePlotRange, handles};
...
handles.menu_plotdb.MenuSelectedFcn = {@callback_toggleplotoption, handles};
...
for ii = 1:4
    ...
    handles.menu_FFTTypeOption(ii).MenuSelectedFcn = {@callback_togglePlotType, handles};
end
for ii = 1:3
...
% handles.axis_fft.Toolbar.Visible = 'off';
% handles.axis_fft.Title.Color = scheme.Axis.AxisColor.Value;
% handles.axis_fft.Title.BackgroundColor = 'none';
handles.axis_fft = uiaxes(...
    'Parent', handles.gl,...
    'Interactions',[],...
    'Color', scheme.Axis.BackgroundColor.Value,...
...
handles.axis_fft.Title.Color = scheme.Axis.AxisColor.Value;
handles.axis_fft.Title.BackgroundColor = 'none';
handles.axis_fft.Layout.Column = 2;
handles.axis_fft.Layout.Row = [2,4];
...
handles.spinner_maxfreq = uispinner(...
    'Parent', handles.panel_plotopts,...
...
handles.menu_FFTtype = uimenu('Parent', handles.menu_plot, 'Label', 'Data type', 'Checked', true);
handles.menu_FFTTypeOption(1) = uimenu('Parent', handles.menu_FFTtype, 'Label', 'Power', 'Checked', true, 'Tag', 'POWER');
handles.menu_FFTTypeOption(2) = uimenu('Parent', handles.menu_FFTtype, 'Label', 'Amplitude', 'Checked', false,'Tag', 'AMP');
handles.menu_FFTTypeOption(3) = uimenu('Parent', handles.menu_FFTtype, 'Label', 'Phase', 'Checked', false,'Tag', 'PHASE');
handles.menu_FFTTypeOption(4) = uimenu('Parent', handles.menu_FFTtype, 'Label', 'ITC', 'Checked', false,'Tag', 'ITC');

handles.menu_autoscale = uimenu('Parent', handles.menu_plot, 'Label', 'Auto scale amplitude', 'Checked', true, 'Separator', 'on');
handles.menu_plotdb = uimenu('Parent', handles.menu_plot, 'Label', 'Plot in dB', 'Checked', true);
