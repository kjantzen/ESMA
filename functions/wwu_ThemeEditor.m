...
% plotter interface
function wwu_ThemeEditor(options)

    arguments
        options.SchemeFile = ''
    end
    
    p = loadScheme(options.SchemeFile);
    if isempty(p)
        return
    end
...
function callback_selectScheme(src, ~, fig)
    ...
    fprintf(src.UserData);
    fig.Name = ['Scheme: ', src.Text];

    p = loadScheme(src.UserData);
    drawEditPanel(h, p);
    setParameters(h,p);
...
function callback_SaveScheme(~,~,f, SaveAsFlag)
    ...
    if SaveAsFlag
        [saveFile, saveLoc] = uiputfile('*.mat','Enter a name for the Scheme File', fullfile(schemeFilePath, h.SchemeFile));
        if saveFile == 0
            return
        end
        h.SchemeFile =fillfile(saveLoc, saveFile);
    end
...
%if no scheme file is passed, populate with the defaults
    if isempty(schemeFile)
        ...
        if ~isfile(schemeFile)
            warning('Could not find the scheme file in the editor folder.  Resorting to defaults!');
            p = InitializeParameters;
        else
            p = load(schemeFile);
        end
...
for cc = length(controls):-1:1
    ...
    for pp = length(props):-1:1
        ...
        switch p.(controls{cc}).(props{pp}).Type
            ...
            case 'Integer'
                uispinner('Parent',parent, 'Step',1,'Limits',[8,100],...
