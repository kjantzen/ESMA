...
W = round(p.screenwidth * .4); H = round(p.screenheight * .5);
...
mycmap = [p.buttoncolor(1),p.buttoncolor(2),p.buttoncolor(3);
...
mp.LineWidth = 2;
line(h.axis_icamean, [0,0], scale, 'Color', 'k')
line(h.axis_icamean, p.EEG.times, zeros(size(p.EEG.times)), 'Color', 'k')
...
h.axis_compbar.Color = p.backcolor;
